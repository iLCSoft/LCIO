 ...
 *
 * @author Tony Johnson
 * @version $Id: SimJob.java,v 1.1 2003-05-06 04:58:55 tonyj Exp $
 */
public class SimJob {
        ...
        for(int rn=0;rn<NRUN;rn++){
            ...
            for(int i=0;i<NEVENT;i++){
                ...
                for(int j=0;j<NHITS;j++){
                    ...
                    double pos[] = { 1.1* random.nextDouble()  , 2.2* random.nextDouble()  , 3.3* random.nextDouble() } ;
                    
                    // fill the extension vector
                    ext.add( 3.14159f ) ;
                    for(int k=0;k<3;k++) ext.add(  (float) pos[k] * 0.1f  ) ;
