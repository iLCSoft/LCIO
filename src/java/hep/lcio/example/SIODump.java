...
public class SIODump
{
	...
	try{ for(;;){ // ever ...
		...
		System.out.println(  "\t block: " + block.getBlockName() 
				     ...
				     + " v" + df.format( block.getMajorVersion() )
				     + "-"  + df.format( block.getMinorVersion() ) ) ;	
	...
	try{ for(int count = 0 ; ; count++){


	    int i = in.readInt() ;

	    if( count % digitsPerLine == 0 ) System.out.print("\t ") ;
	    ...
	    if( count % digitsPerLine == digitsPerLine-1 ) System.out.println() ;
	...
	try{ for(int count = 0 ; ; count++){
	    ...
	    if( count % wordsPerLine == 0 ) System.out.print("\t ") ;
	    ...
	    if( count % wordsPerLine == wordsPerLine-1 ) System.out.println() ;
