...
createLCReader         -> preader = lcrdrcreate()
...
createLCWriter         -> pwriter = lcwrtcreate()
open                   -> status  = lcwrtopen( pwriter, filename )
...
% getParent              -> pmcpp   = lcmcpgetparent( pmcp )
...
getPDG                 -> status  = lcmcpgetpdg( pmcp , ipdg )
...
writeRunHeader         -> status  = lcwriterunheader( pwriter, irun, detname, descrname, sdnamev, nsubd, len(sdnamev(1)) )
readNextRunHeader      -> pheader = lcreadnextrunheader( preader, irun, detname, descrname, sdnamev, nsubd, len(sdnamev(1)) )
...
addSimCaloHit          -> phit    = lcaddsimcalohit( icellid0, icellid1, energy, posv )
addSimCaloHitMCont     -> status  = lcschaddmcparticlecontribution( phit, pmcp, energy, time, ipdg )  (from basic f77 API)
getSimCaloHit          -> phit    = lcgetsimcalohit( pcol, i, icellid0, icellid1, energy, posv )
getSimCaloHitMCont     -> status  = lcgetsimcalohitmccont( phit, i, pmcp, energy, time, ipdg )

for class MCParticle:

getMCParticleData      -> status  = lcgetmcparticledata ( ipdg, ihepevtst, dvtxv, momv, mass, charge, ndaughters )
