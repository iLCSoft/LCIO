...
%\title{LCIO [v1.3beta+] -  Users manual}
\title{{\Huge\bf LCIO  -  Users manual} \\ v1.3beta+}
...
{\bf This document is not yet fully updated to the version 1.3 that is to be released soon.
In particular it does not yet contain the reconstruction data model (Tracks, Clusters and ReconstructedParticles). Please do not use v1.3beta for producing large amounts of data.
}
...
\begin{verbatim}
  mkdir lcio 
  cd lcio
  cvs co -d v1.3beta -r v1.3beta lcio
\end{verbatim}
...
\begin{verbatim} 
  export LCIO=/lcio/v1.3beta                <-- modify as appropriate
...
\begin{verbatim}
  ...
  set LCIO=c:/lcio/devel/v1.3beta            <-- modify as appropriate
...
\begin{verbatim}
  export LCIO=/cygdrive/c/lcio/devel/v1.3beta   <-- modify as appropriate
  export PATH=$LCIO/tools:$LCIO/bin:$PATH
...
%an ANSI compliant C++ compiler installed.
...
\rule[-5mm]{0mm}{10mm} C++ Namespace  &  Java package    &  Purpose \\ \hline \hline
 ...
 the base interfaces that are defined in EVENT. Needed for writing new data or extending exisitng data, e.g.
...
(\#include "lcio.h"). \\ \hline
   \multicolumn{3}{|c|}{\textit{\small Namespaces/packages below are shown for completeness only, don't use in your code!}}         \\ \hline
...
\caption{Overview of the event data model defined by LCIO.
...
All data that is related to one event is stored in these classes. LCRunHeader is used to store run related 
...
\caption{Overview of the data model defined by LCIO (namespace EVENT). 
