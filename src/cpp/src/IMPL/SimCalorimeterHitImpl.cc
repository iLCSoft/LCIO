...
namespace IMPL{
    ...
    SimCalorimeterHitImpl::SimCalorimeterHitImpl(const SimCalorimeterHit& hit) :
        ...
        _energy( hit.getEnergy() ) {
            ...
            for(int i=0; i<nMC ;i++){
                ...
                p = hit.getStepPosition(i) ;
                for(int j=0; j<3 ; j++){
                    con->StepPosition[j] = p[j] ;
                }
    ...
    void SimCalorimeterHitImpl::addMCParticleContribution( EVENT::MCParticle *p,
        ...
        if( ( pdg != 0 && sp == NULL ) || ( pdg == 0 && sp != NULL ) ){
            throw EVENT::Exception(std::string("SimCalorimeterHitImpl::addMCParticleContribution: StepPosition and PDG must both be set in detailed mode " )) ;
        }

        // convenience flag for better code readability
        bool detailedMode = ( pdg != 0 || sp != NULL ) ;


        // add the energy to the SimCaloHit
        _energy += en ;
        ...
        if( ! detailedMode ) { // only one contribution per primary particle (not in detailed mode)

            for( std::vector<MCParticleCont*>::iterator iter=_vec.begin() ; iter != _vec.end() ; iter++ ){
        ...
        for(int i=0; i<3 ;i++){
            if( detailedMode ){
                con->StepPosition[i] = sp[i] ;
            }
            else{
                con->StepPosition[i] = 0 ;
            }
        }
