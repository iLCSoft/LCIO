...
int SIOReader::open(std::string filename){
  ...
  // create SIOHandlers for event headers 
  _evtHandler = new SIOEventHandler( _evtP ) ;

  // tell SIO about it
  SIO_blockManager::add( _evtHandler ) ;
  SIO_blockManager::add( new SIOEventHeaderHandler( _evtP ) ) ;
...
LCEvent* SIOReader::readNextEvent(int accessMode) {
  ...
  if( _stream->getState()== SIO_STATE_OPEN ){
    ...
    *_evtP = new LCEventIOImpl()  ;
    ...
    if( strcmp( _hdrRecord->getName()->c_str() , SIOWriter::HEADERRECORDNAME )  ){
    ...
    *_evtP = new LCEventIOImpl()  ;
    ...
    if( strcmp( _hdrRecord->getName()->c_str() , SIOWriter::EVENTRECORDNAME )){
