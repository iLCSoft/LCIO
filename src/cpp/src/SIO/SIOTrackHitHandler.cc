...
namespace SIO{
  ...
  unsigned int SIOTrackHitHandler::read(SIO_stream* stream, 
				      LCObject** objP,
				      unsigned int flag,
				      unsigned int vers ){
    unsigned int status ; 
    ...
    *objP = hit ;
    ...
    // if we want to point at tracker hits we need to add a ptag:
  ...
  unsigned int SIOTrackHitHandler::write(SIO_stream* stream, 
				       const LCObject* obj,
				       unsigned int flag ){
