...
// Constructor.
// ----------------------------------------------------------------------------
SIO_record::SIO_record(const char* i_name, SIO_verbosity i_verbosity)
...
// ----------------------------------------------------------------------------
unsigned int SIO_record::connect(const char* i_name) {
  //
  // Local variables.
  //
  SIO_block* block;

  std::string s_name = i_name;
  ...
  if (block == NULL) {
    if (verbosity >= SIO_ERRORS) {
      std::cout << "SIO: [/" << name << "/" << i_name << "] "
                << "Block not connected (name not recognized)" << std::endl;
    }
...
// ----------------------------------------------------------------------------
unsigned int SIO_record::connect(SIO_block* block) {
  //
  // Local variables.
  //
  std::string s_name;
  ...
  if (block == NULL) {
    if (verbosity >= SIO_ERRORS) {
      std::cout << "SIO: [/" << name << "/] "
                << "SIO: Block not connected (pointer is NULL)" << std::endl;
    }
    return (SIO_RECORD_BADARGUMENT);
  }
...
// ----------------------------------------------------------------------------
unsigned int SIO_record::connect(std::string* s_name, SIO_block* block) {
  //
  // Local variables.
  //
  std::pair<connectMap_i, bool> status;

  //
  // Initialize the map entry.
  //
  std::pair<std::string const, SIO_block*> entry(*s_name, block);
  ...
  if (status.second) {
    ...
    if (verbosity >= SIO_ALL) {
      std::cout << "SIO: [/" << name << "/" << entry.first << "] "
                << "connected new handler " << std::endl;
    }
  ...
  if (verbosity >= SIO_ALL) {
    for (iter = connectMap.begin(); iter != connectMap.end(); ++iter) {
      std::cout << "SIO: [/" << name << "/" << iter->first << "]  "
                << *(iter->second->getName()) << "  Disconnected" << std::endl;
    }
  }
...
// ----------------------------------------------------------------------------
unsigned int SIO_record::disconnect(const char* i_name) {
  //
  // Local variables.
  //
  SIO_block* block;
  ...
  if (block == NULL) {
    if (verbosity >= SIO_ERRORS) {
      std::cout << "SIO: [/" << name << "/" << i_name << "] "
                << "Not disconnected (block name not recognized)" << std::endl;
    }
    return (SIO_RECORD_BADARGUMENT);
  }
...
// ----------------------------------------------------------------------------
unsigned int SIO_record::disconnect(SIO_block* block) {
  //
  // Local variables.
  //
  std::string s_name;
  ...
  if (block == NULL) {
    if (verbosity >= SIO_ERRORS) {
      std::cout << "SIO: [/" << name << "/] "
                << "Not disconnected (pointer is NULL)" << std::endl;
    }
    return (SIO_RECORD_BADARGUMENT);
  }
  s_name = *(block->getName());
...
// ----------------------------------------------------------------------------
unsigned int SIO_record::disconnect(std::string* s_name, SIO_block* /* block */
) {
  //
  // Local variables.
  //
  ...
  if ((iter = connectMap.find(*s_name)) == connectMap.end()) {
    if (verbosity >= SIO_ERRORS) {
      std::cout << "SIO: [/" << name << "/" << *s_name << "] "
                << "Not disconnected (block is not connected)" << std::endl;
    }
...
// ----------------------------------------------------------------------------
SIO_block* SIO_record::getConnect(const char* i_name) {
  //
  // Local variables.
  //
  connectMap_i iter;
  ...
  if ((iter = connectMap.find(s_name)) == connectMap.end()) {
    if (verbosity >= SIO_ERRORS) {
      std::cout << "SIO: [/" << name << "/" << i_name << "] "
                << "Block is not connected" << std::endl;
    }
    return (NULL);
  }
...
bool SIO_record::getCompress() {
  if ((options & SIO_OPT_COMPRESS) != 0) return (true);
...
// Read a record.
// ----------------------------------------------------------------------------
unsigned int SIO_record::read(SIO_stream* stream, unsigned int i_options) {
  //
  // Local variables.
  //
  SIO_block* block;
  ...
  pointerToMap_i ptoh, ptoi, ptol;

  SIO_POINTER_DECL
  *pointer;

  unsigned int buflen, buftyp, status, tmplen, version;
  ...
  while (stream->buffer < stream->recmax) {
    ...
    if (buftyp != SIO_mark_block) {
      if (verbosity >= SIO_ERRORS) {
        std::cout << "SIO: [" << stream->name << "/" << name << "/] "
                  << "Expected block marker not found" << std::endl;
      }
    ...
    if (tmploc == NULL) {
      if (verbosity >= SIO_ERRORS) {
        std::cout << "SIO: [" << stream->name << "/" << name << "/] "
                  << "Buffer allocation failed" << std::endl;
      }
      return (SIO_STREAM_NOALLOC);
    }

    // SIO_DATA( stream, tmploc, tmplen );
    // fg: keep coverity happy be deleting the temp memory in case of a read
    // error
  ...
  while (ptol != stream->pointerTo->end()) {
    ...
    for (ptoi = ptol; ptoi != ptoh; ptoi++) {
      ...
      *pointer =
          (pat_found ? reinterpret_cast<SIO_POINTER_DECL>(pati->second) : 0);
    }
    ...
    // }
    // else{
    //   std::cout << " ----SIO_record: no pointed At object found for : " <<
    //   ptol->first << std::endl ;
    // }
...
// ----------------------------------------------------------------------------
bool SIO_record::setCompress(bool compress) {
  unsigned int o_options = (options & SIO_OPT_COMPRESS);

  options &= ~SIO_OPT_COMPRESS;
  if (compress) options |= SIO_OPT_COMPRESS;

  if (o_options != 0) return (true);
...
// ----------------------------------------------------------------------------
bool SIO_record::setUnpack(bool i_unpack) {
  bool o_unpack = unpack;
  unpack = i_unpack;

  if (verbosity >= SIO_ERRORS) {
    std::cout << "SIO: [/" << name << "] "
              << " setting unpack from " << o_unpack << " to  " << unpack
              << std::endl;
  }
...
// ----------------------------------------------------------------------------
SIO_verbosity SIO_record::setVerbosity(SIO_verbosity i_verb) {
  SIO_verbosity o_verb = verbosity;
  verbosity = i_verb;
  return (o_verb);
}
...
// Write a record.
// ----------------------------------------------------------------------------
unsigned int SIO_record::write(SIO_stream* stream) {
  //
  // Local variables.
  //
  pointedAtMap_i pati;
  ...
  unsigned int blkver, buflen, match, namlen, status;
  ...
  for (iter = connectMap.begin(); iter != connectMap.end(); iter++) {
    ...
    SIO_functions::copy(UCHR_CAST(&buflen), stream->blkmax, SIO_LEN_QB, 1);
  }
  ...
  */
  ...
  while (ptol != stream->pointerTo->end()) {
    ...
    if (pati != stream->pointedAt->end()) {
      pointer =
          stream->bufloc + reinterpret_cast<SIO_POINTER_DECL>(pati->second);

      SIO_functions::copy(UCHR_CAST(&match), pointer, SIO_LEN_QB, 1);

      for (ptoi = ptol; ptoi != ptoh; ptoi++) {
        pointer =
            stream->bufloc + reinterpret_cast<SIO_POINTER_DECL>(ptoi->second);
